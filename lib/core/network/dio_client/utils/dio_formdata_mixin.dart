...
mixin DioFormdataMixin {
  ...
  // FormData createFormData(dynamic rawData) {
  //   if (rawData is! Map<String, dynamic>) return FormData.fromMap({});
  ...
  //         value,
  //         filename: "00-${DateTime.now().millisecondsSinceEpoch}.png",
  //         contentType: DioMediaType('image', 'png'),
  //       );
  //     } else if (value is File) {
  //       // ✅ Single File → MultipartFile
  //       final filename = value.path.split('/').last;
  //       newData[key] = MultipartFile.fromFileSync(
  //         value.path,
  //         filename: filename,
  //         contentType: DioMediaType('image', 'png'),
  //       );
  ...
  //       final files = value.asMap().entries.map((entry) {
  //         final idx = entry.key;
  //         final bytes = entry.value;
  //         double size = bytes.lengthInBytes / (1024 * 1024);
  //         final filename =
  //             "$key-$idx-${DateTime.now().millisecondsSinceEpoch}.png";
  //         logger("Filename: $filename | Size: $size MB");
  //         return MultipartFile.fromBytes(
  //           bytes,
  //           filename: filename,
  //           contentType: DioMediaType('image', 'png'),
